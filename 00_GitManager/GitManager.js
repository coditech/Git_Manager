...
const getRepositoryURL = async () => {
  try {
    const response = await fetch('https://api.github.com/repos/coditech/Submissions/pulls');
    const data = await response.json();
    const reposURLs = data.map(repo => {
      return [repo.head.repo.clone_url, repo.user.login];
    });
    console.log(reposURLs);
    return reposURLs;
  } catch (e) {
...
const cloneRepos = async () => {
  const pullRequestsRepo = await getRepositoryURL();
  const filteredRepositoryList = pullRequestsRepo.filter(item => {
    const repo_exist = RepositoryList.find(repo => repo[0] === item[0]);
    return repo_exist === undefined;
  });
  const newRepoList = [...RepositoryList, ...filteredRepositoryList];
  newRepoList.forEach(elem => {
